...
var number_regexp = regexp.MustCompile(`(?m)\d+`)

func get_number_indexes(input string) [][]int {
	return number_regexp.FindAllStringIndex(input, -1)
}

func has_adjacent_symbol(input string, word_range []int) bool {
...
func day3_part1() any {
	...
	for _, number_index := range number_indexes {
		if has_adjacent_symbol(string(input), number_index) {
			number, _ := strconv.Atoi(string(input)[number_index[0]:number_index[1]])
			sum += number
		}
	}
